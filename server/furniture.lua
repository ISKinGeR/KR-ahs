...
function CreateFurnitureCallbacks()
    Callbacks:RegisterServerCallback("MLO:PlaceFurniture", function(source, data, cb)
        local char = Fetch:Source(source):GetData("Character")
        if char then
            local insideProperty = GlobalState[string.format("%s:MLO:Property", source)]
                        ...
                        if updated then
                            if _insideProperties[property.id] then
                                for k, v in pairs(_insideProperties[property.id]) do
                                    TriggerClientEvent("MLO:Client:AddItem", k, property.id, #_loadedFurniture[property.id], addedItem)
                                end
                            end
    ...
    Callbacks:RegisterServerCallback("MLO:MoveFurniture", function(source, data, cb)
        local char = Fetch:Source(source):GetData("Character")
        if char then
            local insideProperty = GlobalState[string.format("%s:MLO:Property", source)]
                ...
                if property and (property.keys ~= nil and property.keys[char:GetData("ID")]) and _loadedFurniture[property.id] and property.keys[char:GetData("ID")] ~= nil and (property.keys[char:GetData("ID")].Permissions?.furniture or property.keys[char:GetData("ID")].Owner) then
                    ...
                    if index > 0 then
                        ...
                        if updated then
                            if _insideProperties[property.id] then
                                for k, v in pairs(_insideProperties[property.id]) do
                                    TriggerClientEvent("MLO:Client:MoveItem", k, property.id, data.id, _loadedFurniture[property.id][index])
                                end
                            end
    ...
    Callbacks:RegisterServerCallback("MLO:DeleteFurniture", function(source, data, cb)
        local char = Fetch:Source(source):GetData("Character")
        if char then
            local insideProperty = GlobalState[string.format("%s:MLO:Property", source)]
            if insideProperty and data.id then
                ...
                if property and (property.keys ~= nil and property.keys[char:GetData("ID")]) and _loadedFurniture[property.id] and property.keys[char:GetData("ID")] ~= nil and (property.keys[char:GetData("ID")].Permissions?.furniture or property.keys[char:GetData("ID")].Owner) then
                    ...
                    if updated then
                        if _insideProperties[property.id] then
                            for k, v in pairs(_insideProperties[property.id]) do
                                TriggerClientEvent("MLO:Client:DeleteItem", k, property.id, data.id, _loadedFurniture[property.id])
                            end
                        end
